...
parser.add_argument("--lr-scheduler", type=str, default="constant",
                    help="learning rate scheduler: constant, cosine, or inv_decay")
...
parser.add_argument("--augment", action="store_true",
                    help="enable random-SNR log-mel noise augmentation during training")
parser.add_argument("--specaugment", action="store_true",
                    help="enable SpecAugment (frequency and time masking) during training")
parser.add_argument("--freq-mask-max", type=int, default=8,
                    help="max consecutive mel bands to mask in SpecAugment")
parser.add_argument("--time-mask-max", type=int, default=20,
                    help="max consecutive frames to mask in SpecAugment")
...
parser.add_argument("--vad-pos-weight", type=float, default=1.0,
                    help="positive weight for VAD loss (upweight voiced frames)")
...
def augment_mel_batch(mel_clean, mel_noise, snr_range, device, specaugment=False, freq_mask_max=8, time_mask_max=20):
    """Training-time augmentation: mix clean and noise log-mel, with optional SpecAugment.
    ...
    device : torch.device
    specaugment : bool — whether to apply SpecAugment masking
    freq_mask_max : int — max consecutive mel bands to mask
    time_mask_max : int — max consecutive frames to mask

    Returns
    -------
    Tensor (B, T, N_MELS) — mel passed to the model.
    """
    # Start with noise mixing
    snr_min, snr_max = snr_range
    snr_db = torch.rand(mel_clean.size(0), 1, 1, device=device)
    ...
    if specaugment:
        ...
        # Frequency masking
        for b in range(B):
            if torch.rand(1).item() < 0.5:  # 50% chance per batch item
                f = torch.randint(1, freq_mask_max + 1, (1,)).item()
                f0 = torch.randint(0, F - f + 1, (1,)).item()
                mel_mix[b, :, f0:f0+f] = 0.0

        # Time masking
        for b in range(B):
            if torch.rand(1).item() < 0.5:  # 50% chance per batch item
                t = torch.randint(1, time_mask_max + 1, (1,)).item()
                t0 = torch.randint(0, T - t + 1, (1,)).item()
                mel_mix[b, t0:t0+t, :] = 0.0
...
def train_one_epoch(model, dataloader, optimizer, scheduler, writer,
    ...
    for mel_clean, mel_noise, vad_target, f0_target in pbar:
        ...
        mel_mix = augment_mel_batch(mel_clean, mel_noise, args.snr_range, device, args.specaugment, args.freq_mask_max, args.time_mask_max) if args.augment else mel_clean
        ...
        # ── Loss Computation ──
        # VAD loss: binary cross-entropy with positive weighting
        vad_weights = torch.where(vad_target == 1, args.vad_pos_weight, 1.0)
        vad_loss = (vad_weights * bce(pred_vad.squeeze(-1), vad_target)).mean()
...
def main():
    ...
    elif args.lr_scheduler == "inv_decay":
